...
([[ "${arg_o:?}" = "1" ]] && chmod -R 777 ${DIRECTORY_CONTAINER} && cp -r ${DIRECTORY_HOST_SHARED} ${DIRECTORY_CONTAINER} && ${DIRECTORY_CONTAINER}/${CONTAINER_SCRIPT} -d ${CONTAINER_NAME}) \
  ...
  || ([[ ! -z ${CONFIG_FILE_NAME} ]] && docker run --rm -v ${DIRECTORY_HOST_SHARED}:${DIRECTORY_CONTAINER_SHARED} ${DOCKERHUB_ID}/${CONTAINER_NAME}:${CONTAINER_VERSION} ${DIRECTORY_CONTAINER}/${CONTAINER_SCRIPT} ${CONTAINER_NAME} ${CONFIG_FILE_NAME}) \
  || ([[ $# -eq 0 ]] && docker run --rm -v ${DIRECTORY_HOST_SHARED}:${DIRECTORY_CONTAINER_SHARED} ${DOCKERHUB_ID}/${CONTAINER_NAME}:${CONTAINER_VERSION} ${DIRECTORY_CONTAINER}/${CONTAINER_SCRIPT} ${CONTAINER_NAME})
