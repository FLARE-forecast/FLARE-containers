...
([[ "${arg_o:?}" = "1" ]] && cp -r ${DIRECTORY_HOST_SHARED} ${DIRECTORY_CONTAINER} && ${DIRECTORY_CONTAINER}/${CONTAINER_SCRIPT} -d ${CONTAINER_NAME}) \
  || docker run --rm --user user:user -v ${DIRECTORY_HOST_SHARED}:${DIRECTORY_CONTAINER_SHARED} ${DOCKERHUB_ID}/${CONTAINER_NAME}:${CONTAINER_VERSION} ${DIRECTORY_CONTAINER}/${CONTAINER_SCRIPT} ${CONTAINER_NAME}
