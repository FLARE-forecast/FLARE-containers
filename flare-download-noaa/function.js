...
app.post('/run', function (req, res) {
    ...
    shell.exec(`wget https://raw.githubusercontent.com/FLARE-forecast/FLARE-containers/master/commons/flare_pullworkdir.sh`);
    ...
    if(!process1.status){
        ...
        if(`${payload.container_name}`==`flare-generate-forecast`){
            ...
            shell.cp(`/openwhisk/id_rsa`, `/home/user/.ssh/`);
        }
        ...
        if(!process2.status){
                ...
                if(!process3.status){
                    shell.exec(`wget https://raw.githubusercontent.com/FLARE-forecast/FLARE-containers/master/commons/flare_triggernext.sh`);
                    var next_trigger_payload = JSON.stringify(payload);
                    const process4 = cp.spawnSync('/bin/bash', ['/openwhisk/flare_triggernext.sh', `${payload.openwhisk_apihost}`, `${payload.openwhisk_auth}`, `${payload.container_name}`, `${payload.lake}`, `${next_trigger_payload}`], { stdio: 'inherit' });
                    if(!process4.status){
                        ret += "success";
                    }
                    else{
    ...
    shell.rm('/openwhisk/id_rsa');
    shell.rm('/openwhisk/flare_*');
